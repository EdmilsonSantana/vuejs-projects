<template>
  <v-body>
    <v-section color="black">
      <v-form-group>
        <v-button inverted @click.native="leave">
    ...
    <v-section color="black">
      <v-form inline @submit.prevent.native="sendMessage(message)">
        <v-form-group inline full-width >
          <v-text-field v-model="message" placeholder="Message"></v-text-field>
        </v-form-group>
        <v-form-group inline >
          <v-button inverted type="submit">
            Send Message
          </v-button>
        </v-form-group>
      </v-form>
